...
for e in range(episodes):
    ...
    for robot in env.robot_list:
        agent = Agent(robot)
        mse = agent.read_mse()
        cumulated_mse += mse if mse is not None else 0
        print(f"Episode {e}, Step {i}, Robot {robot.id}, MSE: {(mse / i):.4f}")
    ...
    print(f"Average MSE for episode {e}: {(cumulated_mse / len(env.robot_list) / i):.4f}")
    env.end()
